...
// Timer function
float time_kernel(const std::string& kernel_name, std::function<void()> kernel_launch, int iterations = 100) {
...
int main() {
    ...
    // 1. SAXPY
    time_kernel("saxpy", [&]() {
        saxpy<<< (N + 255) / 256, 256 >>>(2.0f, dA, dB, N * N);
    });
